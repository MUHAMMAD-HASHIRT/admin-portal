...
const ReportEngine = {
    ...
    generateDetailsPage: (s) => { 
        ...
        const avgHt = ReportEngine.calcAvg(s.classId, s.section, 'height'); 
        const avgWt = ReportEngine.calcAvg(s.classId, s.section, 'weight'); 
        const totalAtt = (parseInt(s.attendanceP)||0) + (parseInt(s.attendanceA)||0); 
        const check = (val) => `<span class="dt-check ${val?'checked':''}"></span>`; 

        return `
        <div class="details-page">
            <img src="header footer.png" class="layer-frame">
            <img src="background.png" class="layer-lion">
            <div class="details-content" style="padding-top:140px;">
                <div class="details-header">Student Details</div>
                <table class="details-table-new">
                    <tr><th>Name</th><td colspan="3">${s.name}</td></tr>
                    <tr><th>Parent/Guardian's Name (1)</th><td colspan="3">${s.parent1 || '-'}</td></tr>
    ...
    render: (c, t, m, a) => {
        ...
        const showQuant = t.showQuant !== false;
        const showRemarks = t.showRemarks !== false;
        
        let FOOTER_REQUIRED_SPACE = 20;
        if(showQuant) FOOTER_REQUIRED_SPACE += 100;
        if(showRemarks) FOOTER_REQUIRED_SPACE += 120;
        ...
        if (t.items && t.items.length > 0) {
            ...
            t.items.forEach((i, idx) => { 
                ...
                if (i.type === 'break' || y + h > LIMIT) { 
                    container.appendChild(pg); pg = ReportEngine.pg(); cnt = pg.querySelector('.content-area'); y = 20; 
                    tbl = ReportEngine.createTable(); cnt.appendChild(tbl); tbody = tbl.querySelector('tbody'); y += 40; 
...
const Admin = {
    ...
    loadClassesHierarchy: () => { 
        ...
        DB.data.classes.forEach(cls => {
            try {
                if (!cls.id || !cls.name) return; let subH='';
                if(cls.subjects && Array.isArray(cls.subjects)) { cls.subjects.forEach(sid => { const sb=DB.data.subjects[sid]; if(sb) subH+=`<span class="subj-tag">${sb.name} <button onclick="Admin.deleteSubject('${cls.id}','${sid}')" style="border:none;background:none;color:red;cursor:pointer;">&times;</button></span>` }); }
                if(subH === '') subH='<span style="font-size:11px;color:#999;">No Subj</span>';
                let secH='';
                if (cls.sections && Array.isArray(cls.sections)) { cls.sections.forEach(sec => { secH+=`<div class="tree-section"><div class="tree-sec-name">SECTION ${sec}</div><button onclick="Admin.deleteSection('${cls.id}','${sec}')" class="btn-xs-danger">X</button></div>` }); }
                c.innerHTML+=`<div class="class-tree-item"><div class="tree-header"><span>${cls.name}</span><div><button onclick="Admin.deleteClass('${cls.id}')" class="btn-xs-danger" style="margin-right:5px;">DEL</button><button onclick="Admin.addSectionPrompt('${cls.id}')" class="btn-xs-accent">+SEC</button></div></div><div class="tree-subjects">${subH}</div>${secH}</div>`;
    ...
    addSubject: () => { try { const cid=document.getElementById('subject-class-select').value; const n=document.getElementById('new-subject-name').value.toUpperCase(); if(!cid) { alert("Please Select a Grade first!"); return; } if(!n) { alert("Please enter a subject name!"); return; } const sid='s_'+Date.now(); if(!DB.data.subjects) DB.data.subjects = {}; DB.data.subjects[sid]={name:n,template:DB.createDefaultTemplate(n)}; const c = DB.data.classes.find(x=>x.id===cid); if (!c) { alert("Error: Class ID not found in database."); return; } if (!c.subjects || !Array.isArray(c.subjects)) c.subjects = []; c.subjects.push(sid); DB.save(); document.getElementById('new-subject-name').value=''; Admin.loadClassesHierarchy(); } catch(e) { alert("System Error Adding Subject: " + e.message); } },
    ...
    addTeacher: () => { const n=document.getElementById('new-t-name').value.toUpperCase(); const u=document.getElementById('new-t-user').value; const p=document.getElementById('new-t-pass').value; const e=document.getElementById('new-t-email').value; if(n && u && p) { const exists = DB.data.users.find(x => x.id === u); if (exists) { alert("Error: This Teacher ID already exists! Click 'Auto Generate' again."); return; } DB.data.users.push({id:u, pass:p, role:'teacher', name:n, email:e}); DB.save(); Admin.loadTeachers(); document.getElementById('new-t-name').value = ''; document.getElementById('new-t-user').value = ''; document.getElementById('new-t-pass').value = ''; document.getElementById('new-t-email').value = ''; } else { alert("Please fill Name, ID and Password."); } },
